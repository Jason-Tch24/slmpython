...
tile_size = 15  # a changer
size = 15  # a changer
width, height = size * tile_size, size * tile_size
interface_height = 150  # Hauteur supplémentaire pour l'interface et a changer
...
def AImovement(units, objectives):

    # Filter enemy units
    enemy_units = [unit for unit in units if unit.color == ENEMY_COLOR]

    # Function to get adjacent positions (horizontal, vertical, diagonal)
    def get_adjacent_positions(x, y):
        return [
            (x + 1, y), (x - 1, y), (x, y + 1), (x, y - 1),  # Horizontal and vertical moves
            (x + 1, y + 1), (x - 1, y - 1), (x + 1, y - 1), (x - 1, y + 1)  # Diagonal moves
        ]

    # Function to check if a position is valid (inside map limits and not occupied)
    def is_valid_position(x, y, units):
        return 0 <= x < width and 0 <= y < height and not any(unit.x == x and unit.y == y for unit in units)

    # Function to find the closest objective for a given unit
    def get_closest_objective(unit, objectives):
        min_distance = float('inf')
        closest_objective = None
        for obj in objectives:
            obj_x, obj_y = obj['x'], obj['y']
            distance = abs(unit.x - obj_x) + abs(unit.y - obj_y)
            if distance < min_distance:
                min_distance = distance
                closest_objective = (obj_x, obj_y)
        return closest_objective, min_distance

    # Function to find the best move position for a unit towards an objective
    def get_best_move(unit, objective, units):
        min_move_distance = float('inf')
        best_move = None
        for pos in get_adjacent_positions(unit.x, unit.y):
            move_x, move_y = pos
            if is_valid_position(move_x, move_y, units):
                move_distance = abs(move_x - objective[0]) + abs(move_y - objective[1])
                if move_distance < min_move_distance:
                    min_move_distance = move_distance
                    best_move = (move_x, move_y)
        return best_move

    # Main logic for moving enemy units towards objectives
    for unit in enemy_units:
        if not unit.moved:
            closest_objective, _ = get_closest_objective(unit, objectives)
            if closest_objective:
                best_move = get_best_move(unit, closest_objective, units)
                if best_move:
